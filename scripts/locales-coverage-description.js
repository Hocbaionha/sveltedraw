...
const percentages = fs.readFileSync(
  `${__dirname}/../packages/engine/locales/percentages.json`,
);
...
const printRow = (id, locale, coverage) => {
  ...
  if (locale in crowdinMap && crowdinMap[locale]) {
    result += `[${boldIf(
      language,
      isOver,
    )}](https://example.invalid/translations/${crowdinMap[locale]}) | `;
  } else {
...
console.info(
  `Each language must be at least **${THRESSHOLD}%** translated in order to appear on Sveltedraw. Join us on [Crowdin](https://example.invalid/translations) and help us translate your own language. **Can't find yours yet?** Open an [issue](https://github.com/Hocbaionha/sveltedraw/issues/new) and we'll add it to the list.`,
);
console.info("\n\r");
