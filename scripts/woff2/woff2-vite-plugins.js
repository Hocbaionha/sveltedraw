// define `SVELTEDRAW_ASSET_PATH` as a SSOT
const OSS_FONTS_CDN = "https://excalidraw.nyc3.cdn.digitaloceanspaces.com/oss/";
const OSS_FONTS_FALLBACK = "/";

/**
 * Custom vite plugin for auto-prefixing `SVELTEDRAW_ASSET_PATH` woff2 fonts in `excalidraw-app`.
 *
...
module.exports.woff2BrowserPlugin = () => {
  ...
  return {
    ...
    transform(code, id) {
      ...
      // but at the same time can't be easily prefixed with the `SVELTEDRAW_ASSET_PATH` only for the `excalidraw-app`
      ...
      if (!isDev && id.endsWith("excalidraw-app/index.html")) {
        return code.replace(
          "<!-- PLACEHOLDER:SVELTEDRAW_APP_FONTS -->",
          `<script>
