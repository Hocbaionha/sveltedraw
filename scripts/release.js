 ...
 * - yarn release                                 -> publishes `@sveltedraw` packages with "test" tag and "-[hash]" version suffix
 * - yarn release --tag=test                      -> same as above
 * - yarn release --tag=next                      -> publishes `@sveltedraw` packages with "next" tag and version "-[hash]" suffix
 * - yarn release --tag=next --non-interactive    -> skips interactive prompts (runs on CI/CD), otherwise same as above
 * - yarn release --tag=latest --version=0.19.0   -> publishes `@sveltedraw` packages with "latest" tag and version "0.19.0" & prepares changelog for the release
 *
  ...
  for (const packageName of PACKAGES) {
    ...
    if (pkg.dependencies) {
      for (const dependencyName of PACKAGES) {
        if (!pkg.dependencies[`@sveltedraw/${dependencyName}`]) {
          continue;
        }

        pkg.dependencies[`@sveltedraw/${dependencyName}`] = nextVersion;
      }
    }
    ...
    rl.question(
      ...
      (answer) => {
        ...
        if (answer.toLowerCase() === "y") {
          execSync(`git add -u`);
          execSync(
            `git commit -m "chore: release @sveltedraw/engine@${nextVersion} 🎉"`,
          );
        } else {
          console.warn(
...
const buildPackages = () => {
  ...
  for (const packageName of PACKAGES) {
    console.info(`Building "@sveltedraw/${packageName}"...`);
...
const publishPackages = (tag, version) => {
  for (const packageName of PACKAGES) {
    ...
    console.info(
      `Published "@sveltedraw/${packageName}@${tag}" with version "${version}"! 🎉`,
    );
  }
};
