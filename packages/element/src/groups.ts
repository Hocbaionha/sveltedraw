...
export const selectGroupsForSelectedElements = (function () {
  ...
  let lastSelectedElements: readonly NonDeleted<SveltedrawElement>[] | null =
    null;
  let lastElements: readonly NonDeleted<SveltedrawElement>[] | null = null;
...
export const editGroupForSelectedElement = (
  appState: AppState,
  element: NonDeleted<SveltedrawElement>,
): AppState => {
  return {
    ...appState,
...
export const isElementInGroup = (element: SveltedrawElement, groupId: string) =>
...
export const getElementsInGroup = (
  ...
  const elementsInGroup: SveltedrawElement[] = [];
...
export const addToGroup = (
  prevGroupIds: SveltedrawElement["groupIds"],
  newGroupId: GroupId,
  editingGroupId: AppState["editingGroupId"],
) => {
...
export const removeFromSelectedGroups = (
  groupIds: SveltedrawElement["groupIds"],
  selectedGroupIds: { [groupId: string]: boolean },
...
export const getMaximumGroups = (
  ...
  const groups: Map<String, SveltedrawElement[]> = new Map<
    String,
    SveltedrawElement[]
  >();
  elements.forEach((element: SveltedrawElement) => {
    const groupId =
...
export const getSelectedElementsByGroup = (
  ...
  const groups: Map<string, SveltedrawElement[]> = new Map();
  const elements: Map<string, SveltedrawElement[]> = new Map();

  // helper function to add an element to the elements map
  const addToElementsMap = (element: SveltedrawElement) => {
    // elements
  ...
  const addToGroupsMap = (element: SveltedrawElement, groupId: string) => {
    // groups
    const currentGroupMembers = groups.get(groupId) || [];
    const boundTextElement = getBoundTextElement(element, elementsMap);
