 ...
 *
 * Editor-scope reactive store (one instance per <Sveltedraw> root).
 ...
 * via setContext(EDITOR_STORE_KEY, store) at the <Sveltedraw> root so multiple
 * editors on one page do not share state.
 */

// @ts-ignore — resolved by Vite alias; no tsconfig path to avoid original cascade
 ...
 * The full type lives in packages/engine/types.ts → LibraryItem.
