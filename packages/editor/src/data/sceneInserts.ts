...
export function createSceneInserts(deps: SceneInsertsDeps): SceneInsertsApi {
  ...
  // B1: minimal Frame creation. Original's `frame` element type is already
  ...
  const createFrameAtCenter = (): string | undefined => {
    ...
    const frame: AnyEl = {
      ...
      // with what's visually rendered and what original .sveltedraw JSON files
  ...
  // B4: try to decode the Sveltedraw scene metadata embedded in a PNG. Returns
  ...
  const onContainerPaste = async (event: ClipboardEvent): Promise<void> => {
    ...
    for (const item of Array.from(items)) {
      if (item.type.startsWith("image/")) {
        ...
        if (blob) {
          event.preventDefault();
          // B4: restore embedded Sveltedraw scene metadata if present.
