import { promiseTry, LOCAL_FONT_PROTOCOL } from "@sveltedraw/common";
...
export class SveltedrawFontFace {
  ...
  private static readonly ASSETS_FALLBACK_URL = `https://esm.sh/${
    import.meta.env.PKG_NAME
      ...
      : "@sveltedraw/editor" // fallback to the latest package version (i.e. for app)
  }/dist/prod/`;
  ...
  private static createUrls(uri: string): URL[] | DataURL[] {
    ...
    // absolute assets paths, which are found in tests and app build, won't work with base url, so we are stripping initial slash away
    const assetUrl: string = uri.replace(/^\/+/, "");
    const urls: URL[] = [];

    if (typeof window.SVELTEDRAW_ASSET_PATH === "string") {
      const normalizedBaseUrl = this.normalizeBaseUrl(
        window.SVELTEDRAW_ASSET_PATH,
      );

      urls.push(new URL(assetUrl, normalizedBaseUrl));
    } else if (Array.isArray(window.SVELTEDRAW_ASSET_PATH)) {
      window.SVELTEDRAW_ASSET_PATH.forEach((path) => {
